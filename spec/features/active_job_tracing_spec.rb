...
RSpec.describe "ActiveJob distributed tracing", type: :e2e do
  ...
  def wait_for_trace(timeout: 10)
    ...
    loop do
      ...
      http_txn = transactions.find do |t|
        t.dig("contexts", "trace", "op") == "http.server" &&
          t.dig("transaction") =~ /JobsController#sample_job/
      end
      job_txn = transactions.find { |t| t.dig("contexts", "trace", "op") == "queue.active_job" }

      return [http_txn, job_txn] if http_txn && job_txn
